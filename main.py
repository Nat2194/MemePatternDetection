...
for meme in MEME_PRIORITY:
        ...
        try:
            with open(rule_path, 'r') as f:
                ...
                if content:
                    ...
                    # ADD THIS LINE TO VERIFY WHAT RULE IS LOADED:
                    if meme == "chut":
                        print(f"DEBUG LOADED RULES FOR CHUT: {MEME_RULES[meme]}")
                else:
                    MEME_RULES[meme] = {}
        except Exception as e:
    ...
    image_files = [f for f in os.listdir(meme_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
    if image_files:
...
if os.path.isdir(default_dir):
    default_images = [f for f in os.listdir(default_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
    if default_images:
...
def extract_base_state(results, brow_low_thresh, brow_high_thresh, eye_closed_thresh):
    ...
    if results.face_landmarks:
        ...
        avg_eye = (get_dist(face[159], face[145]) / face_width + get_dist(face[386], face[374]) / face_width) / 2.0
...
while True:
    ...
    if results.face_landmarks: mp_draw.draw_landmarks(frame, results.face_landmarks, mp_holistic.FACEMESH_TESSELATION, green_spec, green_spec)
    if results.left_hand_landmarks: mp_draw.draw_landmarks(frame, results.left_hand_landmarks, mp_holistic.HAND_CONNECTIONS, green_spec, green_spec)
    if results.right_hand_landmarks: mp_draw.draw_landmarks(frame, results.right_hand_landmarks, mp_holistic.HAND_CONNECTIONS, green_spec, green_spec)
    if results.pose_landmarks: mp_draw.draw_landmarks(frame, results.pose_landmarks, mp_holistic.POSE_CONNECTIONS, green_spec, green_spec)
    ...
    if not is_calibrated:
        if results.face_landmarks:
            ...
            brow_history.append((get_dist(face[159], face[52]) / face_width + get_dist(face[386], face[282]) / face_width) / 2)
            eye_history.append((get_dist(face[159], face[145]) / face_width + get_dist(face[386], face[374]) / face_width) / 2.0)
            
            calibration_frames_collected += 1
            cv2.putText(frame, f"CALIBRATING: Keep neutral face... {calibration_frames_collected}/{CALIBRATION_TARGET}", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
            
            if calibration_frames_collected >= CALIBRATION_TARGET:
                avg_resting_brow = sum(brow_history) / len(brow_history)
                dynamic_brow_low, dynamic_brow_high = avg_resting_brow - BROW_LOWER_OFFSET, avg_resting_brow + BROW_RAISE_OFFSET
                dynamic_eye_closed = (sum(eye_history) / len(eye_history)) - EYE_CLOSED_OFFSET
                is_calibrated = True
        else:
    ...
    body_state, current_face_width = extract_base_state(results, dynamic_brow_low, dynamic_brow_high, dynamic_eye_closed)
    ...
    cv2.imshow('Meme Pattern Detector', frame)
    if current_meme: cv2.imshow('Matched Meme', memes[current_meme])
    elif "default" in memes: cv2.imshow('Matched Meme', memes["default"])
    else:
        try: cv2.destroyWindow('Matched Meme')
        except: pass
