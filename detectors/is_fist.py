...
def check_hand_fist(hand_landmarks):
    ...
    return all(get_dist(hand_landmarks.landmark[t], wrist) < (get_dist(hand_landmarks.landmark[m], wrist) * 0.85) for t, m in zip(tips, mcps))
...
def update_state(results, state, face_width):
    ...
    if results.left_hand_landmarks:
        ...
        if pose[15].visibility > 0.4 and pose[19].visibility > 0.4:  # Increased visibility threshold
    ...
    if results.right_hand_landmarks:
        ...
        pose = results.pose_landmarks.landmark
        if pose[16].visibility > 0.4 and pose[20].visibility > 0.4:  # Increased visibility threshold
